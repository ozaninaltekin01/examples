...
async def create_post(post: PostRequest,db: Session = Depends(get_db)):
...
@router.put("/posts/{post_id}",status_code= status.HTTP_200_OK, response_model=PostResponse)
async def update_post(post_id: int, post: PostRequest, db: Session = Depends(get_db)):
